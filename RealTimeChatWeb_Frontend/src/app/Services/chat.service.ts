...
export class ChatService {
  ...
  private baseUrl = 'https://localhost:7180';
  private token:any;
  ...
  private saveMessage(message: any) {
    return this.http.post('https://localhost:7180/api/Chat', message).subscribe();
  }

  public getMessages(fromUser: string, userTo: string) {
    return this.http.get(`https://localhost:7180/api/Chat/${fromUser}/${userTo}`);
  }
  
  public SaveGroupChats(grpMessage: any){
    return this.http.post(`https://localhost:7180/api/Chat/groupChat`, grpMessage);
  }

  public getGroupMessages(groupName: string): Observable<any[]> {
    return this.http.get<any[]>(`https://localhost:7180/api/Chat/getGroupMessages/${groupName}`);
  }

  public unreadCount(){
    return this.http.get<any[]>(`https://localhost:7180/api/Seen/messages/unread-counts`);
  }

  public lastMessage(userName:string){
    return this.http.get<any[]>(`https://localhost:7180/api/Chat/lastMessages/${userName}`);
  }
